    ...
    async def initiate_call(
        ...
        **kwargs: Any,
        ...
        if to_number.startswith("SIP/") or to_number.startswith("PJSIP/"):
            # Normalize endpoint name based on the SIP domain in the dial string
            # Trunk domain uses vobiz-trunk (creds: dailsmart / Reddy@7989)
            # Registrar domain uses vobiz-endpoint (creds: dailsmart9645...)
            if "455bdb01.sip.vobiz.ai" in to_number:
                # Replace any endpoint name with vobiz-trunk for the SIP trunk domain
                import re
                sip_endpoint = re.sub(
                    r"^PJSIP/[^/]+/",
                    "PJSIP/vobiz-trunk/",
            ...
            # Clean number to check if it's a phone number
            clean_digits = "".join(c for c in to_number if c.isdigit())
            # If it's a phone number (e.g. >= 6 digits), route via the registered Vobiz trunk
            if len(clean_digits) >= 6:
                sip_endpoint = f"PJSIP/vobiz-trunk/sip:{clean_digits}@455bdb01.sip.vobiz.ai"
            else:
                # Default to PJSIP technology for extensions
        ...
        if from_number:
            params["callerId"] = from_number.lstrip("+")
