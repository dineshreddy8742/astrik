...
class ARIConnection:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.request(method, url, auth=auth, **kwargs) as response:
                ...
                if response.status not in (200, 201, 204):
                    error_msg = f"{method} {path} -> {response.status}: {response_text}"
                    logger.error(f"[ARI org={self.organization_id}] REST API error: {error_msg}")
    ...
    async def _create_external_media(
        ...
        try:
            ...
            if "501" in str(e):
                logger.error(
                    f"[ARI org={self.organization_id}] externalMedia failed with 501 Not Implemented. "
                    "This usually means your Asterisk version is too old. "
                    "Dograh requires Asterisk 20.16.0+, 21.11.0+, or 22.6.0+ for WebSocket transport."
                )
            raise e
